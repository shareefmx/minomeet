...
export const RecordingScreen: React.FC = () => {
  ...
  const getSpeakerBadgeStyle = (speaker?: string) => {
    const s = (speaker || 'You (Microphone)').toLowerCase();
    if (s.includes('mic') || s.includes('you') || s.includes('1')) {
      return 'bg-[#eff6ff] text-[#1e40af] border border-[#bfdbfe]';
    } else if (s.includes('system') || s.includes('remote') || s.includes('participant') || s.includes('meeting') || s.includes('2')) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative overflow-hidden bg-white text-[#111827]">
      {/* Top Header Bar */}
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-8 py-6 space-y-3 pb-28">
        {liveTranscript.length === 0 && !interimTranscript ? (
          ...
          <>
            ...
            {interimTranscript && (
              <div className="flex items-start gap-3 text-[14.5px] leading-relaxed bg-[#f0fdf4] border border-[#bbf7d0] p-3 rounded-xl shadow-2xs animate-in fade-in duration-75 mt-3">
                <span className="text-xs text-[#16a34a] font-mono font-bold select-none pt-0.5 min-w-[45px] flex items-center gap-1">
                  <span className="w-2 h-2 rounded-full bg-[#16a34a] animate-ping" />
                  Live
                </span>
                <div className="flex-1">
                  <span className="font-bold text-[#15803d] mr-2 text-[11px] uppercase tracking-wide bg-[#dcfce7] px-2 py-0.5 rounded border border-[#86efac]">
                    Speaking
                  </span>
